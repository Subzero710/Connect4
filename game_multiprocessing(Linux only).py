...
def alpha_beta_decision(board, turn, depth, queue, max_player):
    ...
    grid = np.array(board.grid, copy=True)
    tasks = [(grid, depth, move, max_player, -WIN, WIN) for move in moves]

    best_move = moves[0]
    best_val = -WIN

    for move, val in _MP_POOL.imap_unordered(root_worker, tasks, chunksize=1):
        if val > best_val:
...
def root_worker(args):
    grid, depth, move, max_player, alpha, beta = args
    b = Board()
    b.grid = np.array(grid, copy=True)
...
if os.name != "nt":  # Linux/WSL
    ...
    _MP_POOL = ctx.Pool(processes=min(7, os.cpu_count() or 1))
